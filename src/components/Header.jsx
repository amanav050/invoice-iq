          ...
          <nav className="flex bg-white/[0.03] border border-white/[0.06] rounded-xl p-1" role="tablist">
            {tabs.map((tab) => (
              <button key={tab.id} role="tab" aria-selected={activeTab === tab.id}
                ...
                className={`px-4 sm:px-5 py-1.5 text-[13px] font-medium rounded-lg transition-all duration-300 ${activeTab === tab.id ? 'bg-accent/10 text-accent shadow-sm' : 'text-muted-light hover:text-text'}`}>
